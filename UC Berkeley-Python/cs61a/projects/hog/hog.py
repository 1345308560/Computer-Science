...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if score0 > score1:
        ...
        leader = None 
    if leader == None or last_leader == leader:
        return leader,message
    else:
        message = "Player "
        message += str(0) if leader == 0 else str(1)
        message += " takes the lead by "
        message += str(score0-score1) if leader == 0 else str(score1-score0)
        return leader,message
    # END PROBLEM 6
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    while cnt<11 :
        ...
        temp = averaged_dice(cnt,dice)
        if temp > Max_Num:
            Result = cnt
            Max_Num = temp
        # print(cnt,": ",averaged_dice(cnt,dice))
        cnt += 1 
    return Result
    # END PROBLEM 9
...
def pigs_on_prime_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    new_score = score + oink_points(score, opponent_score)
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    return min(oink_points_strategy(score, opponent_score),pigs_on_prime_strategy(score,opponent_score))
